...
class Webhook extends \Opencart\System\Engine\Controller
{
    public function subscribe(): void
    {
        ...
        if (empty($body['callback_url'])) {
            $this->emit(['status' => 422, 'body' => \Shopwalk\Ucp\Response::error('missing_callback_url', 'callback_url is required')]);
            return;
        }
        ...
        $this->emit(['status' => 201, 'body' => \Shopwalk\Ucp\Response::ok($row)]);
    }
    ...
    public function unsubscribe(): void
    {
        ...
        $subs = new \Shopwalk\Ucp\WebhookSubscriptions($this->registry);
        $subs->delete($id);
        $this->emit(['status' => 200, 'body' => \Shopwalk\Ucp\Response::ok(['id' => $id, 'status' => 'disabled'])]);
    }
    ...
    public function list(): void
    {
        ...
        $subs = new \Shopwalk\Ucp\WebhookSubscriptions($this->registry);
        $this->emit(['status' => 200, 'body' => \Shopwalk\Ucp\Response::ok(['subscriptions' => $subs->listAll()])]);
    }
