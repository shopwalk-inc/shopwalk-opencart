...
class Oauth extends \Opencart\System\Engine\Controller
{
    ...
    {
        $server = new \Shopwalk\Ucp\OauthServer($this->registry);
    ...
    public function register(): void
    {
        ...
        if (empty($body['name']) || empty($body['redirect_uris'])) {
            $this->emit(['status' => 422, 'body' => \Shopwalk\Ucp\Response::error('invalid_request', 'name and redirect_uris required')]);
            return;
        }
        ...
        $this->emit(['status' => 201, 'body' => \Shopwalk\Ucp\Response::ok([
